...
| D-0654 | 2026-04-13 | fix | P0 face enrollment gap: photo upload on onboarding page never triggered face descriptor extraction for ANY staff type (MO or contractor). StaffMOTab had auto-enroll on photo upload but only for MO staff; onboarding page had zero face enrollment. Fix: (1) Created lib/face/extract-descriptor.ts — reusable client-side utility (face-api.js model load + descriptor extraction + auto-enroll API call). (2) Wired autoEnrollFace into onboarding AddStaffForm + EditStaffForm — fires after photo upload + staff creation/edit for all staff types. (3) Added "Batch Enroll Faces" button on onboarding staff page — iterates all staff with photo_url but no face enrollment, loads each image client-side, extracts descriptor, calls /api/face/enroll. Shows progress counter. For CHV backfill of 40 staff with photos but face_enrolled=false. |
